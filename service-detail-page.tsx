...
import { getServicesForLocale, type ServiceItem } from "@/lib/services-data";
import type { Locale } from "@/lib/i18n/locale";
import { getDictionary } from "@/lib/i18n/dictionary";
import { SERVICO_TO, servicoAltPaths } from "@/lib/i18n/routes";

export function ServiceDetailPage({ locale, service }: { locale: Locale; service: ServiceItem }) {
  ...
  return (
    <main className="impakta-page">
      <ImpaktaNav locale={locale} altPaths={servicoAltPaths(service.slug)} />

      {/* Same "vídeo de capa" reel as the homepage hero, compact and looping
          — every service page keeps the branded video running. */}
      <CoverVideoHero
        compact
        eyebrow={dict.serviceDetail.eyebrow(String(index + 1).padStart(2, "0"))}
        ...
        <section className="impakta-service-gallery">
          <div className="impakta-service-gallery__inner">
            <p className="impakta-eyebrow">
              ...
              {dict.serviceDetail.galleryEyebrow}
            </p>
      ...
      <section className="impakta-service-other">
        <div className="impakta-service-other__inner">
          <p className="impakta-eyebrow">
            <span aria-hidden="true" className="impakta-crosshair" />
            {dict.serviceDetail.otherEyebrow}
          </p>
          <div className="impakta-service-other__grid">
            {other.map((s) => (
              <Link className="impakta-service-other__item" key={s.slug} params={{ slug: s.slug }} to={SERVICO_TO[locale]}>
