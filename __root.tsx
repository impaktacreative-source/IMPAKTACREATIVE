...
import { HTML_LANG, localeFromPathname } from "../lib/i18n/locale";
...
import { IMPAKTA_THEME_COLOR } from "../lib/impakta-theme-color";
...
// IMPAKTA head kit (favicon/apple-touch-icon/manifest/theme-color) — the full
// set generated in Phase 2 (see asset-system.md §7), always served from this
// app's own /public, so these paths are safe to hardcode alongside the
// per-field app-meta.json values above.
const HEAD_KIT_LINKS = [
...
function buildHead(meta: AppMeta) {
  ...
  return {
    meta: [
      ...
      { name: "author", content: "IMPAKTA Creative" },
      { name: "theme-color", content: IMPAKTA_THEME_COLOR },
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-q-background-primary px-4">
      <NotFound
        ...
        subtitle="Zero impakto encontrado aqui. A página que procuras não existe ou foi movida."
      >
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-q-background-primary px-4">
      <div className="max-w-md text-center">
        <h1 className="text-q-title-lg-semi-bold text-q-text-primary">Esta página não carregou</h1>
...
function RootShell({ children }: { children: ReactNode }) {
  // The IMPAKTA site lives at three locale-prefixed URL trees ("/", "/en",
  // "/es"); <html lang> must track whichever one is currently rendering, for
  // accessibility and SEO. Derived from the router's own pathname so it's
  // correct on the very first SSR pass, no client-only flash.
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const locale = localeFromPathname(pathname);

  return (
    <html lang={HTML_LANG[locale]} data-theme="default-dark" style={{ colorScheme: "dark" }}>
      {/* Marketplace apps are permanently dark: data-theme is pinned on <html>
          ...
          toggle, or a light mode. The IMPAKTA page itself renders its own
          light editorial palette via .impakta-page, independent of this. */}
      <head>
        <HeadContent />
      </head>
